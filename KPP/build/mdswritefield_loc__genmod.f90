        !COMPILER-GENERATED INTERFACE MODULE: Tue Jan 10 17:25:44 2017
        MODULE MDSWRITEFIELD_LOC__genmod
          INTERFACE 
            SUBROUTINE MDSWRITEFIELD_LOC(FNAME,FILEPREC,GLOBALFILE,     &
     &ARRTYPE,NNZ,ARR,IRECORD,MYITER,MYTHID)
              INTEGER(KIND=4) :: NNZ
              CHARACTER(*) :: FNAME
              INTEGER(KIND=4) :: FILEPREC
              LOGICAL(KIND=4) :: GLOBALFILE
              CHARACTER(LEN=2) :: ARRTYPE
              REAL(KIND=8) :: ARR(-3:29,-3:29,NNZ,1,1)
              INTEGER(KIND=4) :: IRECORD
              INTEGER(KIND=4) :: MYITER
              INTEGER(KIND=4) :: MYTHID
            END SUBROUTINE MDSWRITEFIELD_LOC
          END INTERFACE 
        END MODULE MDSWRITEFIELD_LOC__genmod
