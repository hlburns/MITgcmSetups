...
c      Real*8 recip_hFacI(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
...
c     &     ,recip_hFacI
     ...
     &     mxlSurfFlag
      ...
      Real*8 mskCor(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
...
C     TKEPrandtlNumber :: here, an empirical function of the Richardson number
      ...
      Real*8     uStarSquare
      Real*8     verticalShear
      Real*8     KappaM, KappaH
c     Real*8     Nsquare
...
c         tempU= .5D0*( uVel(i,j,km1,bi,bj)+uVel(i+1,j,km1,bi,bj)
c     &                 -( uVel(i,j,k  ,bi,bj)+uVel(i+1,j,k  ,bi,bj)) )
c     &        *recip_drC(k)
c         tempV= .5D0*( vVel(i,j,km1,bi,bj)+vVel(i,j+1,km1,bi,bj)
c     &                 -( vVel(i,j,k  ,bi,bj)+vVel(i,j+1,k  ,bi,bj)) )
c     &        *recip_drC(k)
c         verticalShear = tempU*tempU + tempV*tempV
c         RiNumber   = MAX(Nsquare(i,j,k),0.D0)/(verticalShear+GGL90eps)
cC     compute Prandtl number (always greater than 0)
c         prTemp = 1.D0
c         IF ( RiNumber .GE. 0.2D0 ) prTemp = 5.D0 * RiNumber
c         TKEPrandtlNumber(i,j,k) = MIN(10.D0,prTemp)
C     mixing length
...
c        rMixingLength(i,j,k) = 1.D0 /GGL90mixingLength(i,j,k)
c       ENDDO
c      ENDDO
c     ENDDO
