...
C     == Local variables ==
      ...
      Real*8 tmpFac, uBarC, vBarC, tmph
      INTEGER i,j,K,bi,bj
      INTEGER km1
...
C--   fill momentum state-var diagnostics:
     ...
     &          +uVel(i,j,K,bi,bj)*drF(K)*hFacW(i,j,K,bi,bj)
                tmph = tmph+drF(K)*hFacW(i,j,K,bi,bj)  
              ENDDO
              tmp1k(i,j,bi,bj) = tmp1k(i,j,bi,bj)/tmph
            ENDDO
           ENDDO
          ENDDO
         ENDDO
         CALL DIAGNOSTICS_FILL(tmp1k,'SDIAG1   ',0,1,0,1,1,myThid)
        ENDIF

        IF ( DIAGNOSTICS_IS_ON('SDIAG2   ',myThid) ) THEN
         DO bj = myByLo(myThid), myByHi(myThid)
          DO bi = myBxLo(myThid), myBxHi(myThid)
           DO j = 1,sNy+1
            DO i = 1,sNx
              ...
              DO K = 1,Nr
                tmp1k(i,j,bi,bj) = tmp1k(i,j,bi,bj)
     &          +vVel(i,j,K,bi,bj)*drF(K)*hFacW(i,j,K,bi,bj)
                tmph = tmph+drF(K)*hFacW(i,j,K,bi,bj)  
              ENDDO
              tmp1k(i,j,bi,bj) = tmp1k(i,j,bi,bj)/tmph
            ENDDO
          ...
          ENDDO
         ENDDO
         CALL DIAGNOSTICS_FILL(tmp1k,'SDIAG2   ',0,1,0,1,1,myThid)
        ENDIF

        IF ( DIAGNOSTICS_IS_ON('SDIAG3   ',myThid) ) THEN
         DO bj = myByLo(myThid), myByHi(myThid)
          DO bi = myBxLo(myThid), myBxHi(myThid)
           DO j = 1,sNy
            DO i = 1,sNx+1
     ...
     &          +uVel(i,j,K,bi,bj)*drF(K)*hFacW(i,j,K,bi,bj)
              ...
              ENDDO
              tmp1k(i,j,bi,bj) = (tmp1k(i,j,bi,bj)/tmph)
     &                          *(tmp1k(i,j,bi,bj)/tmph) 
            ENDDO
           ENDDO
          ENDDO
         ENDDO
         CALL DIAGNOSTICS_FILL(tmp1k,'SDIAG3   ',0,1,0,1,1,myThid)
        ENDIF

        IF ( DIAGNOSTICS_IS_ON('SDIAG4   ',myThid) ) THEN
         DO bj = myByLo(myThid), myByHi(myThid)
          DO bi = myBxLo(myThid), myBxHi(myThid)
           DO j = 1,sNy+1
            DO i = 1,sNx
     ...
     &                          *(tmp1k(i,j,bi,bj)/tmph) 
          ...
          ENDDO
         ENDDO
         CALL DIAGNOSTICS_FILL(tmp1k,'SDIAG4   ',0,1,0,1,1,myThid)
        ENDIF
